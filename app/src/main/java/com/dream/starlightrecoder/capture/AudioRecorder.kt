package com.dream.starlightrecoder.capture
...
/**
 * Continuously records audio and notifies the [AudioRecorder.Callback] when voice (or any
 * sound) is heard.
 *
 *
 ...
 */
class AudioRecorder(private val mCallback: Callback) {
    ...
    private fun createAudioRecord(): AudioRecord? {
        for (sampleRate in SAMPLE_RATE_CANDIDATES) {
            val sizeInBytes = AudioRecord.getMinBufferSize(
                sampleRate,
                CHANNEL,
                ENCODING
            )
    ...
    companion object {
        ...
        private const val AMPLITUDE_THRESHOLD = 450
        private const val SPEECH_TIMEOUT_MILLIS = 500
        private const val MAX_SPEECH_LENGTH_MILLIS = 4 * 1000
        private val isPause =
            AtomicBoolean(false)
    }
